...
public interface Movie_ScheduledRepository extends JpaRepository<Movie_Scheduled, Integer>{
	@Query("SELECT mc FROM Movie m, Movie_Scheduled mc WHERE mc.ID_MOVIE = m.ID and mc.DATE_END >= GETDATE()-1 and m.ID=?1")
	List<Movie_Scheduled> listMovieScheduledByIdMovie(Integer idMovie);
	
	@Query("SELECT CAST(mc.DATE_END AS java.sql.Date) FROM Movie m, Movie_Scheduled mc WHERE mc.ID_MOVIE = m.ID and mc.DATE_END >= GETDATE()-1 and m.ID=?1")
